...
Application::Application(int& argc, char** argv) : QApplication(argc, argv)
{
    ...
    // Initialize application settings
    {
        ...
        m_settings->registerSetting("LicenseKey", QString());
        ...
        // --- DRM: Initialize license manager ---
        m_licenseManager = std::make_unique<LicenseManager>(this);
        QString licenseKey = m_settings->get("LicenseKey").toString();
        if (!licenseKey.isEmpty()) {
            m_licenseManager->startMonitoring(licenseKey);
            qInfo() << "License monitoring started.";
        } else {
            qWarning() << "No license key found – DRM disabled.";
        }
    }
                         ...
                         const QString& offlineName)
{
    if (m_updateRunning) {
        ...
        controller->setLicenseManager(m_licenseManager.get());
        if (window) {
