...
{
    return static_cast<ModLoaderType>(
...
{
    return s_indexed_version_type_names.value(
        type,
        IndexedVersionType::Unknown
    );
}
...
{
    return ((provider == ModPlatform::ResourceProvider::FLAME)
...
QString DependencyTypeUtils::toString(DependencyType type)
{
    switch (type) {
        ...
        case DependencyType::OPTIONAL_DEPENDENCY:
            return "OPTIONAL";
...
DependencyType DependencyTypeUtils::fromString(const QString& str)
{
    static const QHash<QString, DependencyType> map = {
        ...
        { "OPTIONAL", DependencyType::OPTIONAL_DEPENDENCY },
