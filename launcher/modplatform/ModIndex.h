...
struct IndexedVersionType {
    ...
    constexpr IndexedVersionType(Enum e = Enum::Unknown) : m_type(e) {}

    static IndexedVersionType fromString(const QString& type);

    bool isValid() const
    {
        return m_type != Enum::Unknown;
    }

    std::strong_ordering operator<=>(const IndexedVersionType& other) const = default;

    std::strong_ordering operator<=>(const IndexedVersionType::Enum& other) const
    {
        return m_type <=> other;
    }
    ...
    explicit operator IndexedVersionType::Enum()
    {
        return m_type;
    }
...
struct IndexedVersion {
    ...
    {
        auto release_type = version_type.isValid()
            ? QString(" [%1]").arg(version_type.toString())
            : "";

        auto versionStr = !version.contains(version_number)
            ? version_number
            : "";
...
struct IndexedPack {
    ...
    bool isAnyVersionSelected() const
    {
        ...
        return std::any_of(
            ...
            versions.constEnd(),
            [](const auto& v) {
                return v.is_currently_selected;
            }
        );
    }
};
